...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while cap.isOpened():
        ...
        try:
            ...
            # counter logic
            if angle < 60:  # down angle gives some leeway so the connection doesnt have to be exactl 180 to be counted as down
                stage = "down"
            if angle > 150 and stage == 'down':  # up angle ensures that the rep doesnt have to go all the way to the top
                stage = "up"
                counter += 1
                print(counter)
        except:
