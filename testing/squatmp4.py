...
# === INIT SETUP ===
input_path = 'testing/squat.mp4'          # Path to input video
output_path = 'testing/output.mp4'        # Path to save output video

cap = cv2.VideoCapture(input_path)

# Get width, height, and FPS for output video
    ...
    while cap.isOpened():
        ...
        try:
            ...
            cv2.putText(image, posture_status,
                        tuple(np.multiply(left_hip, [
                              width, height - 10]).astype(int)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, posture_color, 2)
